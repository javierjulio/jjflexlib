package com.javierjulio.flexlib.controls
{
	...
	public class TextHeader extends Text
	{
		//--------------------------------------------------------------------------
		//
		//  Class variables
		...
		{
			if (!StyleManager.getStyleDeclaration("TextHeader"))
			{
				// there is no CSS definition for TextHeader so set default values
				...
				StyleManager.setStyleDeclaration("TextHeader", styles, false);
			}
			return true;
		}
		...
		override protected function updateDisplayList(unscaledWidth:Number, unscaledHeight:Number):void 
		{
			...
			if (currentIcon) 
			{
				...
				// when there is more than 1 line of text just align the icon with 
				// the very first line of text, otherwise vertically center it
				currentIcon.move(x, (textField.numLines > 1) 
										? y + UITextField.TEXT_HEIGHT_PADDING 
										: (unscaledHeight - currentIcon.height) / 2);
